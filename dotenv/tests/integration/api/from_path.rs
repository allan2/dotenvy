...
fn no_file() {
    ...
    test_in_env(&testenv, || {
        let err = from_path("nonexistent.env").unwrap_err();
...
fn empty_custom_file() {
    ...
    test_in_env(&testenv, || {
        assert!(from_path(".custom.env").is_ok());
    });
}
