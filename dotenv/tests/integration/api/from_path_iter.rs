...
fn no_file_err() {
    let testenv = TestEnv::init();
    test_in_env(&testenv, || match from_path_iter("nonexistent") {
...
fn empty_default_file() {
    ...
    test_in_env(&testenv, || {
        from_path_iter(".env").unwrap().for_each(|_| {
...
fn empty_custom_file() {
    ...
    test_in_env(&testenv, || {
        from_path_iter(".custom.env").unwrap().for_each(|_| {
            panic!("should have no keys");
        });
    });
}
...
fn default_file_not_read_on_missing_file() {
    test_in_default_env(|| {
        from_path_iter("nonexistent.env").ok();
...
fn two_vars_into_hash_map() {
    check_iter_default_envfile_into_hash_map(|| from_path_iter(".env"));
}

fn assert_single_key_file(path: impl AsRef<Path>) {
    let (key, value) = from_path_iter_unwrap_one_item(path);
    assert_eq!(key, KEY_1);
    assert_eq!(value, VAL_1);
}

fn from_path_iter_unwrap_one_item(path: impl AsRef<Path>) -> (String, String) {
    from_path_iter(path)
        .expect("valid file")
        .next()
        .expect("one item")
