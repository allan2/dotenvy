...
impl TestEnv {
    ...
    ///
    /// Setting it to an empty string will cause an empty env file to be created
    pub fn set_envfile_contents(&mut self, contents: impl ToString) -> &mut Self {
        self.env_file_contents = Some(contents.to_string());
        self
    }
    ...
    ///
    /// If `None` is returned, an env file will not be created
    pub fn envfile_contents(&self) -> Option<&str> {
        self.env_file_contents.as_deref()
    }
...
impl Default for TestEnv {
    fn default() -> Self {
        ...
        Self {
            ...
            work_dir,
            env_vars,
            env_file_contents: envfile_contents,
            env_file_path: envfile_path,
        }
    }
}
...
unsafe fn create_env(test_env: &TestEnv) {
    // only create the env file if its contents has been set
    if let Some(contents) = test_env.envfile_contents() {
        create_env_filefile(&test_env.env_file_path, contents);
    }
...
fn create_env_filefile(path: &Path, contents: &str) {
    if path.exists() {
        panic!("env file `{}` already exists", path.display())
    }
    ...
    if let Err(err) = create_env_file_inner(path, contents) {
        ...
        panic!("error creating env file `{}`: {}", path.display(), err);
    }
}
